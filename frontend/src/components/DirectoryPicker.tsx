...
import type { DirectoryUi } from '../lib/uiState'
...
export function DirectoryPicker({ value, state, onStateChange, onSelect, onClose }: { value: string | null; state?: DirectoryUi; onStateChange?: (state: DirectoryUi) => void; onSelect: (path: string | null) => void; onClose: () => void }) {
  const dialog = useRef<HTMLDialogElement>(null)
  const requestId = useRef(0)
  const [listing, setListing] = useState<DirectoryListing | null>(null)
  const [path, setPath] = useState(state?.pathInput ?? value ?? '')
  ...
  const navigate = useCallback(async (next?: string, input?: string) => {
    ...
    try {
      ...
      setListing(result)
      setPath(input ?? result.path)
      stateCallback.current?.({ currentPath: result.path, pathInput: input ?? result.path })
  ...
  useEffect(() => {
    ...
    void navigate(initialState.current?.currentPath || value || undefined, initialState.current?.pathInput)
  ...
  return <dialog ref={dialog} className="modal directory-modal" aria-labelledby="directory-title" onCancel={event => { if (nativePending) event.preventDefault(); else onClose() }} onClick={event => { if (event.target === dialog.current && !nativePending) onClose() }}>
    ...
    <form className="directory-path-form" onSubmit={event => { event.preventDefault(); if (!nativePending) void navigate(path.trim() || undefined) }}><input aria-label="Путь к папке" value={path} onChange={event => { setPath(event.target.value); stateCallback.current?.({ currentPath: listing?.path || null, pathInput: event.target.value }) }} placeholder="Путь к папке" disabled={nativePending} /><button className="button button-secondary" disabled={loading || nativePending}>Открыть</button></form>
