...
import { ArrowDown, ArrowUpRight, Check, CirclePause, GitBranch, Layers2, Link2, LoaderCircle, Maximize, Minus, Plus, Square, Trash2, X } from 'lucide-react'
...
export function WorkspaceView({ workspace, settings, anyRunning, messageEvents, reconnect, refresh, onUpdate, onSettingsUpdate, notify }: { workspace: Workspace; settings: Settings; anyRunning: boolean; messageEvents: { sequence: number; value: Message }[]; reconnect: number; refresh: (wid: string) => Promise<void>; onUpdate: (workspace: Workspace) => void | Promise<void>; onSettingsUpdate: (settings: Settings) => void | Promise<void>; notify: (text: string, error?: boolean) => void }) {
  ...
  async function run(ids?: string[]) {
    ...
    try { await mutate(() => api.start(wid, ids)) } finally { setActionPending(false) }
  }
  ...
  return <div className="workspace-view">
        ...
        <div className="workspace-content"><div className="graph-area" tabIndex={0} aria-label="Граф тасклетов" onKeyDown={event => { const target = event.target as HTMLElement; if (['INPUT', 'TEXTAREA', 'SELECT'].includes(target.tagName) || target.isContentEditable) return; const focusedNode = target.closest<HTMLElement>('.react-flow__node'); if ((event.key === 'Enter' || event.key === ' ') && focusedNode?.dataset.id) { event.preventDefault(); selectNode(focusedNode.dataset.id); return } if ((event.key === 'Delete' || event.key === 'Backspace') && !locked && selectedEdgeId) { event.preventDefault(); void mutate(() => api.deleteEdge(wid, selectedEdgeId)).then(() => setSelectedEdgeId(null)).catch(() => {}) } }}>
          ...
          {!workspace.tasklets.length && <div className="canvas-state empty-state"><div className="empty-illustration" aria-hidden="true"><div className="empty-mini-node"><span /><i /><i /></div><div className="empty-node-connector"><span /><ArrowDown size={14} /></div><div className="empty-main-node"><span className="empty-node-icon"><Layers2 size={18} strokeWidth={1.6} /></span><div><span /><i /></div><span className="empty-node-dot" /></div><div className="empty-floating-dot" /></div><span className="eyebrow">МЕСТО ДЛЯ СЛЕДУЮЩЕЙ ИДЕИ</span><h2>Всё начинается<br />с одного тасклета.</h2><p>Опишите задачу, соедините её с другими<br className="desktop-break" /> и дайте идеям двигаться дальше.</p><button className="button button-primary" onClick={openCreate}><Plus size={16} />Создать первый тасклет</button><span className="empty-hint">Один тасклет — одна задача и свой чат</span></div>}
