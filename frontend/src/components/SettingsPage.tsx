...
export function SettingsPage({ settings, workspace, globalLocked, workspaceLocked, onUpdate, onWorkspaceUpdate, notify }: { settings: Settings; workspace: Workspace | null; globalLocked: boolean; workspaceLocked: boolean; onUpdate: (settings: Settings) => void | Promise<void>; onWorkspaceUpdate: (workspace: Workspace) => void | Promise<void>; notify: (text: string, error?: boolean) => void }) {
  ...
  return <main ref={container} className="settings-page" onScroll={event => patchSettingsUi(wid, { scroll: event.currentTarget.scrollTop })}>
    <div className="settings-intro"><span className="eyebrow">ВАШЕ ПРОСТРАНСТВО, ВАШИ ПРАВИЛА</span><h1>Настройки</h1><p>{workspace ? workspace.name : 'Общие настройки aispace'}</p></div>
    {workspace && <form className="settings-form workspace-settings-form" onSubmit={event => { event.preventDefault(); void saveWorkspace() }}>
      <section className="settings-section"><div className="section-heading"><div><h2>Настройки пространства</h2><p>Только для «{workspace.name}».</p></div></div>
        ...
        <p className="field-help">{settings.execution_mode === 'codex' ? 'Тасклеты наследуют эту папку, если не выбрана своя.' : 'Папки доступны Codex. Режим API сам не читает локальные файлы.'}</p>
        ...
        <div className="settings-footer inline-settings-footer"><span>{workspaceDirty ? 'Есть несохранённые изменения' : 'Все изменения сохранены'}</span><button className="button button-primary" disabled={workspaceDisabled || !workspaceDirty}>{pending === 'workspace' ? <LoaderCircle size={14} className="spin" /> : <Check size={14} />}Сохранить пространство</button></div>
      </section>
    </form>}
    <form onSubmit={event => { event.preventDefault(); void saveGlobal() }} className="settings-form">
      {globalLocked && <div className="notice">Общие настройки доступны после остановки всех выполняющихся задач.</div>}
      <section className="settings-section">
        <div className="section-heading"><span className="section-icon"><KeyRound size={18} /></span><div><h2>Подключение к AI</h2><p>Общее для всех пространств.</p></div></div>
        ...
        {mode === 'api' ? <>
          ...
          <p className="field-help"><ShieldCheck size={13} />Ключ хранится на backend. Новый несохранённый ключ потребуется ввести заново после обновления страницы.</p>
        </> : <>
        ...
        <label className="field">Модель по умолчанию<input value={model} onChange={event => setGlobal({ model: event.target.value })} placeholder={mode === 'codex' ? 'Автоматически — модель Codex по умолчанию' : 'Идентификатор модели у вашего провайдера'} disabled={disabled} /></label>
      ...
      <section className="settings-section"><div className="section-heading"><div><h2>Выполнение задач</h2><p>Общие ограничения и доступ к файлам.</p></div></div>
        {mode === 'codex' && <><label className="field">Доступ к файлам<select value={sandbox} onChange={event => setGlobal({ codex_sandbox: event.target.value as Settings['codex_sandbox'] })} disabled={disabled}><option value="read-only">Только чтение</option><option value="workspace-write">Чтение и изменение в рабочей папке</option></select></label><p className="field-help">{sandbox === 'read-only' ? 'Для изучения проекта и подготовки предложений.' : 'Тасклеты смогут изменять файлы и выполнять команды в рабочей папке.'}</p></>}
        <label className="field parallel-field">Параллельные задачи<select value={parallel} onChange={event => setGlobal({ max_parallel: Number(event.target.value) })} disabled={disabled}>{[1, 2, 3, 4, 5, 6, 7, 8].map(value => <option key={value} value={value}>{value}</option>)}</select></label><p className="field-help">Лимит тасклетов внутри одного пайплайна. Одновременно работает одно пространство.</p>
        {mode === 'codex' && <p className="field-help command-help">{codex?.capabilities.commands.includes('/goal') ? '/goal <цель> — начать цель; /goal pause, resume, clear — управление. /stop — остановить пайплайн этого пространства.' : codex?.capabilities.commands.length ? `Команды исполнителя: ${codex.capabilities.commands.join(', ')}.` : 'Доступность команд зависит от подключённого исполнителя Codex.'}</p>}
      </section>
      <div className="settings-footer"><span>{globalDirty ? 'Есть несохранённые изменения' : 'Все изменения сохранены'}</span><button className="button button-primary" disabled={disabled || !globalDirty}>{pending === 'global' ? <LoaderCircle className="spin" size={15} /> : <Check size={15} />}{pending === 'global' ? 'Сохраняем…' : 'Сохранить настройки'}</button></div>
    </form>
