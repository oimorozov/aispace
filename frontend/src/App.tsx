...
export default function App() {
  ...
  return <div className={`app-shell ${ui.sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
    <aside className="sidebar" aria-label="Боковая панель">
      ...
      <button className="icon-button sidebar-toggle" aria-label={ui.sidebarCollapsed ? 'Развернуть боковую панель' : 'Свернуть боковую панель'} data-tooltip={ui.sidebarCollapsed ? 'Развернуть боковую панель' : 'Свернуть боковую панель'} aria-expanded={!ui.sidebarCollapsed} onClick={() => updateUi(current => ({ ...current, sidebarCollapsed: !current.sidebarCollapsed }))}>{ui.sidebarCollapsed ? <PanelLeftOpen size={17} /> : <PanelLeftClose size={17} />}</button>
      <nav aria-label="Основная навигация"><button aria-label="Пространство" data-tooltip="Пространство" aria-current={page === 'workspace' ? 'page' : undefined} className={`nav-item ${page === 'workspace' ? 'active' : ''}`} onClick={() => navigate('workspace')}><Workflow size={18} /><span>Пространство</span><span className="nav-count">{workspace?.tasklets.length || 0}</span></button><button aria-label="Настройки" data-tooltip="Настройки" aria-current={page === 'settings' ? 'page' : undefined} className={`nav-item ${page === 'settings' ? 'active' : ''}`} onClick={() => navigate('settings')}><Settings2 size={18} /><span>Настройки</span></button></nav>
