...
test('tasklets can be created, edited, restored, and deleted', async ({ page, request }, testInfo) => {
  await page.goto('/')
  await expect(page.getByRole('button', { name: 'Создать тасклет', exact: true })).toBeVisible()
  ...
  await page.screenshot({ path: testInfo.outputPath('tasklet-draft.png'), fullPage: true })
...
test('navigation, settings, and tasklet creation remain usable on a narrow screen', async ({ page }, testInfo) => {
  ...
  await page.getByRole('button', { name: 'Новый тасклет', exact: true }).click()
  await page.screenshot({ path: testInfo.outputPath('mobile-create-tasklet.png'), fullPage: true })
  ...
  await page.getByRole('tab', { name: /^Чат/ }).click()
  await page.screenshot({ path: testInfo.outputPath('mobile-empty-chat.png'), fullPage: true })
})
